describe('Films Tests for User', () => {
  ...
  it('Should redirect to the details page of the film and should see a button say "Buy Now" when a non-purchased film card is clicked', () => {
    cy.get('body').then(($body) => {
      const $responseWhenEmpty = $body.find('#response-when-empty');
      if ($responseWhenEmpty.length === 0) {
        // Navigate to the last page
        cy.get('#pagination-top a').eq(-2).click();

        cy.get('#results a')
          .eq(-1)
          .then(($el) => {
            const hasAbsoluteDiv = $el.find('div.absolute').length > 0;

            if (!hasAbsoluteDiv) {
              cy.wrap($el).click();

              cy.contains('button', 'Buy Now').should('exist');
            }
          });
      }
    });
  ...
  it('Should redirect to the details page of the film and should see a button say "Watch Film" when a purchased film card is clicked', () => {
    cy.get('body').then(($body) => {
      const $responseWhenEmpty = $body.find('#response-when-empty');
      if ($responseWhenEmpty.length === 0) {
        cy.get('#results a')
          ...
          .then(($el) => {
            ...
            if (hasAbsoluteDiv) {
              ...
              cy.contains('a', 'Watch Film').should('exist');
            }
          });
      }
    });
